...
$query = $db->query("SELECT * FROM _S9Q_countries ORDER BY name ASC");
 ...
 $filename = get_template_directory_uri()."/Assets/Javascript/callingcodes.json";
 
 $data = file_get_contents($filename);
 $array = json_decode($data, true);

 foreach($array as $row) {
      $SQL = $db->query("INSERT INTO _S9Q_callcodes (countryName, code, phoneCode) VALUES('".$row["countryName"]."','".$row["code"]."','".$row["phoneCode"]."')");
 }
...
<div class="row">
    ...
    <div class="form-group">
      ...
      <select>
        ...
        <?php
        $data = file_get_contents("callingcodes.json")
        ?>
      </select>
